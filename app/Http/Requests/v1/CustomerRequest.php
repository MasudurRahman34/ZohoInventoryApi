...
class CustomerRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        if ((request()->routeIs('customers.store')) || (request()->routeIs('customers.update'))) {
            $rules;
        } else {
            $rules += [
                //bill address
                ...
                //primary contact
                'primary_contact.salutation' => ['string', "regex:/^[\p{L} ,.'-]+$/u", 'between:2,50', 'nullable'],
                'primary_contact.first_name' => ['string', "regex:/^[\p{L} ,.'-]+$/u", 'between:3,50', 'nullable'],
                'primary_contact.last_name' => ['string', "regex:/^[\p{L} ,.'-]+$/u", 'between:3,50', 'nullable'],
                'primary_contact.display_name' => 'string|between:3,50|nullable',
                'primary_contact.company_name' => 'string|between:3,255|nullable',
                'primary_contact.phone_number_country_code' => ['nullable', 'regex:/^\+\d{1,3}$/', 'between:2,6'],
                ...
                //
                'other_contact.*.salutation' => ['string', "regex:/^[\p{L} ,.'-]+$/u", 'between:2,50', 'nullable'],
                'other_contact.*.first_name' => ['string', "regex:/^[\p{L} ,.'-]+$/u", 'between:3,50', 'nullable'],
                'other_contact.*.last_name' => ['string', "regex:/^[\p{L} ,.'-]+$/u", 'between:3,50', 'nullable'],
                ...
                'other_contact.*.phone_number_country_code' => ['nullable', 'regex:/^\+\d{1,3}$/', 'between:2,6'],
